...
void AudioBeatAnalyzerApp::computeLogSoundEnergy(const vector<float> &magSpectrum)
{
	...
	for (int i = 0; i < 11; i++)
	{
		...
		if (i == mBand0) {
			sensibilityCoef = mBeatSensitivityCoef[0]; beatThreshold = mBeatThreshold[0]; magnitudecoef = mMagnitudeOutCoef[0];
		...
		if (i == mBand1) {
			sensibilityCoef = mBeatSensitivityCoef[1]; beatThreshold = mBeatThreshold[1]; magnitudecoef = mMagnitudeOutCoef[1];
		...
		if (i == mBand2) {
			sensibilityCoef = mBeatSensitivityCoef[2]; beatThreshold = mBeatThreshold[2]; magnitudecoef = mMagnitudeOutCoef[2];
